...
class ClampChatUI:
    def __init__(self, root, runtime):
        ...
        # streaming 缓冲 queue (worker 线程 put, 主线程周期性 drain)
        # 解决 stutter 高频 token 时 root.after(0,...) idle 队列爆炸假死
        ...
        self.max_new_tokens_var = tk.StringVar(value="1200")

        # 猫娘神经元 (A=L33#6417 推喵, B=L34#12188 抑制喵; 整列权重重写, 独立机制)
        self.meow_enabled_var = tk.BooleanVar(value=False)
        self.meow_alpha_var = tk.StringVar(value="3.0")  # step32b 硬喵实测甜点
        self.meow_beta_var = tk.StringVar(value="0.0")   # 默认关 B (硬喵+anti-stutter 已够)
        self.meow_block_repeat_var = tk.BooleanVar(value=True)  # 防 "喵喵" 连续 stutter

        self.neuron_rows = []
        self.preset_var = tk.StringVar(value="baseline")

        self._build_layout()
        # 启动周期性 drain (50ms ~= 20Hz, stutter 不会撑爆)
        self.root.after(50, self._drain_stream_queue)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    break
            if chunks:
                self._append_stream("".join(chunks))
        finally:
            self.root.after(50, self._drain_stream_queue)
    ...
    def _build_layout(self):
        ...
        meow_frame = ttk.LabelFrame(right, text="猫娘口癖 (A=L33#6417 推, B=L34#12188 抑)", padding=8)
        meow_frame.grid(row=1, column=0, sticky="ew", pady=(8, 0))
        meow_frame.columnconfigure(1, weight=1)
        ttk.Checkbutton(meow_frame, text="启用喵神经元 (A+B)",
                        variable=self.meow_enabled_var).grid(row=0, column=0, columnspan=2, sticky="w")
        ttk.Label(meow_frame, text="ALPHA (A 推喵)").grid(row=1, column=0, sticky="w", pady=(8, 0))
        ttk.Entry(meow_frame, textvariable=self.meow_alpha_var, width=12).grid(row=1, column=1, sticky="w", pady=(8, 0))
        ttk.Label(meow_frame, text="β (B 抑制, 0=关 B)").grid(row=2, column=0, sticky="w", pady=(4, 0))
        ttk.Entry(meow_frame, textvariable=self.meow_beta_var, width=12).grid(row=2, column=1, sticky="w", pady=(4, 0))
        ttk.Checkbutton(meow_frame, text="阻断连续喵 (符号层防 stutter)",
                        variable=self.meow_block_repeat_var).grid(
            row=3, column=0, columnspan=2, sticky="w", pady=(6, 0))

        preset_frame = ttk.LabelFrame(right, text="预设组合", padding=8)
        preset_frame.grid(row=2, column=0, sticky="ew", pady=(8, 0))
    ...
    def _collect_snapshot(self):
        ...
        try:
            meow_alpha = float(self.meow_alpha_var.get().strip())
        except ValueError:
            raise ValueError("猫娘 ALPHA 必须是数字")
        try:
            meow_beta = float(self.meow_beta_var.get().strip())
        except ValueError:
            raise ValueError("猫娘 β 必须是数字")
        ...
        return {
            ...
            "meow_enabled": bool(self.meow_enabled_var.get()),
            "meow_alpha": meow_alpha,
            "meow_beta": meow_beta,
            "meow_block_repeat": bool(self.meow_block_repeat_var.get()),
        }
    ...
    def on_send(self):
        ...
        def stream_cb(text):
            # 走 queue, 由主线程周期性批量 drain — 避免高频 after(0,...) 撑爆 idle 队列假死
        ...
        def worker():
            try:
                if compare:
                    ...
                    base_snapshot["meow_enabled"] = False
